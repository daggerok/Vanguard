/**
 * Headless harness for the single-file Vanguard UI (index.html).
 *
 * The application ships as one inline <script> inside index.html. This
 * harness extracts that script and evaluates it inside a node:vm context
 * wired to a deliberately small fake DOM, localStorage, IndexedDB, and a
 * file-backed fetch() that serves the real generated feed from
 * api/vanguard/. That lets bun test exercise the exact production code
 * (selection writers, sort persistence, watchlist aggregation, paging)
 * without a browser.
 */
...
const API_ROOT = path.join(REPO_ROOT, "api", "vanguard");
 ...
 * select-all checkboxes, tab buttons). It is NOT a DOM parser — it only
 * extracts elements the tests need to see.
 */
...
function createFeedFetch(stats: FetchStats) {
  return async function fetchStub(input: string): Promise<any> {
    ...
    if (!relative.startsWith("api/vanguard/")) return notFound();
...
export type AppHandle = {
  ...
  domLoadedListeners: Array<(event: any) => void>;
};
...
export async function createApp(options: { storage?: MemoryStorage } = {}): Promise<AppHandle> {
  ...
  const scripts = [...htmlSource.matchAll(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/g)]
    .map((match) => match[1])
  ...
  const documentShim = {
    ...
    addEventListener(type: string, fn: any) {
      if (type === "DOMContentLoaded") domLoadedListeners.push(fn);
    },
    removeEventListener() {},
  ...
  vm.runInContext(appScript, ctx, { filename: "index-inline.js" });

  const run = <T = unknown>(code: string): T => vm.runInContext(code, ctx) as T;

  const boot = async () => {
    if (!domLoadedListeners.length) throw new Error("DOMContentLoaded listener was not registered");
    for (const listener of domLoadedListeners) await listener({});
  };
...
/** Treats blank / dash / N/A style placeholders as missing (spec §5). */
